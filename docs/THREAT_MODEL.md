...
**Prevent:**
- Hand-typed trust-tier escalation. `Guard.from_token()` requires an encoded, HMAC-signed token and re-verifies it — a caller cannot simply pass `trust_tier="remote.microvm"` as a string and be believed. *(An earlier version accepted a bare, hand-constructable `Token` object, defeating the requirement; that is fixed.)*
- Self-signed tokens from an empty or unset secret. `sign()`/`verify()` reject an empty secret. *(The original code accepted `secret=b""` silently.)*
...
**Prevent:**
- Destructive tool calls (`rm -rf`, `DROP TABLE`, force-push, etc.) via deterministic, first-match-wins regex policy. A policy with no explicit `default` is rejected at load — no silent fallback. Matching runs on RE2 (linear-time by construction), not stdlib `re` — a policy-author-written pattern matched against attacker-controlled content cannot be forced into catastrophic backtracking, a real DoS this project used to be exposed to (see below).
...
**Real false positives/negatives and their resolution** (see `Evaluation.md`):
...
- A plausible, non-exotic policy-author pattern (`(\w+)+\d`) hung the process for 5+ seconds on a 31-byte adversarial payload via catastrophic backtracking -- the matching engine itself was the exposed surface, not any one shipped pattern. Fixed by migrating `arg_patterns` matching from stdlib `re` to RE2, which structurally cannot backtrack catastrophically; not a heuristic detector layered on top of the old engine.
...
**Detect:** `Guard.call()` writes a `release` event to the configured sink before dispatch and a `terminal` event when dispatch returns or raises. Both carry the same random `call_id` and SHA-256 `call_digest`; the digest binds agent ID, trust tier, tool, exact JSON arguments, and the full policy verdict. If the sink retains the release and the process dies before the terminal write, the release remains pending. `unresolved_releases(records)` lists release IDs that have no valid terminal event in the supplied stream. An interrupted dispatch that can unwind records a terminal `unknown` outcome. `ApprovalRequest` carries the same call ID and digest; the approver must return an `ApprovalGrant` matching both, so boolean approvals and stale/cross-call grants fail closed. The grant correlates the response but does not authenticate the human or approval service; that remains the trusted adapter's job. Arguments are detached from the dispatch copy, and mutation of the approver's copy causes approval to fail closed.

This narrows the missing-evidence window for calls through `Guard.call()` and `guarded()`. It does not prove that every attempted action reached the guard: a producer can suppress both events, and an operator can delete or replace the whole audit stream. A hard interruption in the narrow gap after an allow decision but before the release event is written can also leave no release record. The approval-request event is written before invoking the human callback, so an interrupted prompt is visible. If persisting the release event fails, dispatch is not called. Repeated Ctrl-C during audit fan-out still stops fan-out by design.
...
`executed` attests **release**, never **completion**. In lifecycle records, `outcome="returned"` means only that dispatch returned without raising; it does not prove that a remote side effect committed. `raised` means the boundary observed an exception, whose side effects may still be uncertain. `unknown` means dispatch did not return normally and the side-effect outcome is unknown. The governing invariant:

- A non-null `error` means the boundary observed a failure; the side effect may still have occurred.
...
This is why the shipped tool functions raise: `agent_guard.cli._shell` runs with `check=True` and `agentguard_identity.runtime._run` likewise, so a non-zero exit or a signal death reaches the record as a `subprocess.CalledProcessError` string — an exit status ("returned non-zero exit status 42") reads differently from a signal ("died with `<Signals.SIGKILL: 9>`"). Lifecycle fields are optional in `AuditRecord`; when they are absent, signing omits them so existing per-record HMACs remain verifiable.

**The boundary that cannot honour terminal reporting:** the MCP proxy records at *forward* time (`agent_guard/mcp.py`, immediately after `guard.decide`), because it hands the `tools/call` to the downstream server and never sees the result. For an MCP-produced record a null `error` means **unknown**, not "no failure was observed." The release/terminal lifecycle and reconciler do not yet cover MCP responses.
...
**Prevent:** malformed-but-syntactically-valid JSON (a bare array, `null`, a non-string `tool`) previously reached unvalidated code paths and crashed with an unhandled traceback that leaked absolute filesystem paths to stderr; this now fails closed with a clean, documented error (exit 1) instead.
...
- **Prompt injection causing the agent to construct a plausible-looking but genuinely novel malicious call** is not eliminated by a deterministic ruleset — only mitigated by keeping the ruleset narrow, tested, and paired with isolation as a second layer. `examples/guarded_autonomous_agent/agent_injection_scenario.py` tests this directly: a prompt-injection payload planted in a file the agent reads as ordinary due diligence, with the task prompt narrowed so the injection is the strongest explanation for a read of the restricted file — not strictly the only one, since locating the target file requires a directory listing that also reveals the restricted file's existence (existence-knowledge is not the same as a motive). Across two live runs, the agent's own alignment (Claude Opus) recognized and refused the injected instruction both times, so this scenario has NOT demonstrated an injection reaching a real tool call — that remains unverified. What IS true regardless, by construction: `Policy.evaluate()` takes only `(tool, args, trust_tier)` — it has no parameter for the agent's reasoning — so it cannot distinguish a manipulated call from a directly-instructed one, which is exactly what `agent.py`'s direct-instruction test already proves unconditionally.
...
- **The project has never been published or run against real-world adversarial traffic.** Nothing here has adversarial-in-the-wild experience.
