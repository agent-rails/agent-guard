...
- **The policy engine is blind to volume and sequence.** `Policy.evaluate(tool, args, trust_tier)` has no session state — no call counter, no velocity tracking, no notion of "this tool called 500 times" or "a read of X followed by a write to Y." It gates each call in isolation and correctly, deterministically allows or denies that one call — but it cannot express or catch machine-speed accumulation of individually-in-policy actions. This is the same shape as the Hugging Face intrusion cited elsewhere in this project's docs as validation (17,600 actions, no single one exotic): agent-guard would pass every one of those calls individually. Isolation and egress-deny provide partial mitigation by containing blast radius, but the policy and audit layers themselves do not block or flag rate. The smallest sound fix is a thin, optional stateful counting/velocity layer beside the existing audit sink — not a change to the deterministic RE2 engine, which stays correct for what it does.
