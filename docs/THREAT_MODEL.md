...
**Detect:** a released call terminated by a `BaseException` (for example `KeyboardInterrupt` or `SystemExit`) is still recorded, with `executed=True` and a typed `error` stating that dispatch did not return and the side-effect outcome is unknown. A Ctrl-C landing mid-dispatch previously left no record at all despite the tool having already run — that record loss is closed **for the dispatch window specifically**: from the moment the guard releases the call to the moment dispatch returns or raises. Two record-loss windows remain open on either side of it, both reachable by a real Ctrl-C: an interrupt during `decide()` (including inside the interactive approval prompt, the most likely place an operator hits Ctrl-C) leaves no record, and an interrupt after dispatch returns but before the success-path `record()` completes also leaves no record despite the tool having run. A third gap is operator-initiated and deliberate: interrupting a *second* time while the audit write itself is running aborts the fan-out (`MultiAuditSink`), so any sink not yet reached does not get the record. A repeated Ctrl-C is treated as an instruction to stop that outranks completing the write, because an operator who cannot terminate a security tool is the worse failure. The record asserts the call was released, not that it completed; an operator must treat the side effect as possibly-applied.
